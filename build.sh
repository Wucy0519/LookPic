...
swiftc -sdk "$SDK" -target "$TARGET" -O -whole-module-optimization \
  -framework AppKit -framework QuartzCore -framework ImageIO -framework UniformTypeIdentifiers \
  -o "$APP/Contents/MacOS/看图" \
  "$ROOT/Sources/Catalog.swift" \
  "$ROOT/Sources/Resizer.swift" \
  "$ROOT/Sources/Viewer.swift" \
  "$ROOT/Sources/AppMain.swift"

cp "$ROOT/Support/Info.plist" "$APP/Contents/Info.plist"
codesign --force --sign - "$APP"
"$APP/Contents/MacOS/看图" --self-test
echo "已生成 $APP"
